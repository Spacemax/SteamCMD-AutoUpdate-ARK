...
cd /d E:\Servers
:: Setup SSL cert - Define location of SSL certificate to allow twitter notifications
set SSL_CERT_FILE=E:\Servers\cacert.pem
...
set install_directory=E:\Servers\ARK

:: This is for the directory where you installed steamcmd ::
:: http://media.steampowered.com/installer/steamcmd.zip ::
set steamcmd_path=E:\Servers\steamcmd\steamcmd.exe
...
set exe_path=E:\Servers\ARK\ShooterGame\Binaries\Win64\ShooterGameServer.exe "TheIsland?SessionName=7Gamers ARK AU2 TREX?QueryPort=28015?ServerAdminPassword=NotThis?listen"
...
:: Default value 300 seconds 5 minute (the more frequent the more chance of a steam error making the script think a patch is out) ::
set interval=300
...
title %servername%  SteamCMD ARK Updater
...
rem %curl% -o %~n0-latest-version.txt ""http://api.steampowered.com/ISteamUserStats/GetSchemaForGame/v2/?key=%steamkey%^&appid=%update_appid%^&format=json""
%steamcmd_path% +app_info_update 1 "346110" +app_info_print "346110" +quit >%~n0-latest-version.txt
...
rem execute updater and then close when updates installed and validated, uses script now autosteam.txt
rem %steamcmd_path% +login adamld85 L3tme1n^!now +force_install_dir %install_directory% +app_update %appid% validate +quit
%steamcmd_path% +runscript autosteam.txt
